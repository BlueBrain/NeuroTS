...
def _create_section_spatial_grower(process_type):
    ...
    step_size_distribution.draw_positive = Mock(
        side_effect = itertools.cycle([0.1, 0.2])
    )
...
def test_section_spatial_grower__construction():
    ...
    npt.assert_allclose(grower.direction, [0., 1., 0.])

    npt.assert_equal(len(grower.morphology_points.data), 3)

    npt.assert_allclose(grower.history(), [0., 0., 0.])
    npt.assert_allclose(grower.last_point, [0., 1., 2.])
...
def test_section_spatial_grower__first_point():
    ...
    expected_point = [0., 1.1, 2.]

    npt.assert_allclose(grower.direction, [0., 1., 0.])
    npt.assert_allclose(grower.latest_directions, [[0., 1., 0.]])
    npt.assert_allclose(grower.points, [[0., 1., 2.],
                                        expected_point])

    npt.assert_allclose(grower.morphology_points.data, [[0., 1., 1.],
                                                        [1., 0., 0.],
                                                        [0., 1., 0.],
                                                        expected_point])
...
def test_section_spatial_grower__next_direction():
    ...
    npt.assert_allclose(grower._neighbor_contribution(current_point), [0., 0., 1.])

    npt.assert_allclose(grower.history(), [0., 1., 0.])
    npt.assert_allclose(grower.next_direction(current_point), [0., 1., 0.])
...
def test_section_spatial_grower__next_point():
    ...
    expected_next_direction = 0.5 * grower.direction + 0.5 * np.array([0., 0., 1.])
    expected_next_direction /= np.linalg.norm(expected_next_direction)

    expected_next_point = np.array([0., 1.1, 2.]) + 0.2 * expected_next_direction

    npt.assert_allclose(grower.latest_directions, [[0., 1., 0.],
                                                   expected_next_direction])
