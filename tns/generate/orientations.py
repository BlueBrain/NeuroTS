...
from tns.morphmath import sample, rotation
...
class OrientationManagerBase:
    ...
    Args:
        ...
        soma (Soma): TNS Soma
        parameters (dict): TNS parameters dict
        distributions (dict): TNS distributions dict
    ...
    def _collect_mode_methods(self):
        """Collects and stores in self._modes the methods, the name of which
        starts with '_mode_'
    ...
    def compute_tree_type_orientations(self, tree_type):
        """Computes the orientations for all tree types. It updates the _orientations
        dictionary.
...
class OrientationManager(OrientationManagerBase):
    """Class for dealing with the generation of the tree orientations
    starting from the soma of the cell.
    ...
    def _mode_use_predefined(self, values_dict, tree_type):  # pylint: disable=no-self-use
        ...
        return normalize_vectors(
            np.asarray(values_dict['orientations'], dtype=np.float64)
        )
...
def spherical_angles_to_orientations(phis, thetas):
    ...
    Returns:
        np.array: The orientation vectors where each row
            correspnds to a phi-theta pair.
    """
    return np.column_stack((
        np.cos(phis) * np.sin(thetas),
