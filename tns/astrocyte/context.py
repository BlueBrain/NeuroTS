...
class EndfeetTargets:
    """Endfeet targets are the 3D points that influence the growth of the
    targeted tree growth. A tree will grow towards these points as long as
    they remain active. If a point is deactivated then it will no longer
    affect the grower it is assigned to.
...
class SpaceColonizationContext:
    ...
    def __init__(self, params):
        ...
        if field['type'] == 'logit':
            ...
            raise TNSError(f"{field['type']} function type is not available")
        ...
        if 'point_cloud' in sc_params:
            ...
            raise TNSError('point_cloud entry is not available in params')
    ...
    def kill_distance(self, segment_length):
        """ Space colonization algorithm kill distance. Space colonization
        requires the removal of seed points (point cloud) around the newly
        created points. Kill distance is the threshold radius of the ball query
        to find the points to remove.
    ...
    def influence_distance(self, segment_length):
        """ Space colonization algorithm influence distance. Space colonization
        requires finding the seeds (point cloud) around points that will influence
        the growth direction. Influence distance is the threshold radius of the
