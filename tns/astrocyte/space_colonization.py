...
ENDFOOT_SQUARED_DISTANCE = 5.


def _majorize_process(process, stop, target_distance):
    '''Curates the non-major processes to apply a gradient to large components. If the
    expected maximum length is larger that target distance, then a process type of major
...
def _repulsion(points, current_point, length_constant):
    """Calculate the repulsion from the points to the ref_point. The length_constant
    determines the decay rate of the exponential contribution.
...
def _fallback_strategy(section_direction, angles, repulsion):
    """Directional splitting. dir1 is always same as section_direction
    plus the repulsion. dir2 is determined by the angles and the repulsion
...
def _colonization_split(section, angles, parameters, context):
    ...
    if section.process == 'major':
        ...
        seed_ids = point_cloud.partial_ball_query(
            current_point, influence_distance, section_direction, np.pi / 3., np.pi / 2.5)
        ...
        seed_ids = point_cloud.upper_half_ball_query(current_point, influence_distance, section_direction)

        if len(seed_ids) < 2:
            dir1, dir2 = _fallback_strategy(section_direction, angles, repulsion)
        else:
            vectors_to_seeds = point_cloud.points[seed_ids] - current_point
            dir1, dir2 = _colonization_strategy_secondary(
                vectors_to_seeds, repulsion)

    if np.allclose(dir1, dir2):

        L.warning('Splitting directions are identical. Use of fallback strategy to recalculate them.')
        dir1, dir2 = _fallback_strategy(section_direction, angles, repulsion)

    return dir1, section.process, dir2, 'secondary'


def _add_attraction_bias(current_point, target_point, max_target_distance,
                         direction, attraction_function):
    """Adds a bias to the direction, the magnitude of which depends on the attraction
    function. The latter takes as an input the ratio of the distance of the current
    ...
    """
    target_direction, magnitude = from_to_direction(
        current_point, target_point, return_length=True)

    fraction = clip(magnitude / max_target_distance, 0.0, 1.0)
    a = clip(attraction_function(fraction), 0.0, 1.0)

    new_direction = a * target_direction + (1. - a) * direction
    ...
    if in_squared_proximity(current_point, target_point, ENDFOOT_SQUARED_DISTANCE):
        ...
        if process1 == 'major':
            dir1 = _add_attraction_bias(
                current_point, target_point, max_target_distance, dir1, section.context.field)
        else:

            process1 = _majorize_process(
                process1, section.stop_criteria['TMD'],
...
class SpaceColonization(TMDAlgo):
    """Algorithm for growing a tree using a variation of the space colonization
    algorithm. A spatial colonization context is required, which will provide access
    to the seed point cloud for the respective queries needed for the splitting strategies.

    The tns space colonization algorithm operates using the following data:
    ...
    def bifurcate(self, current_section):
        ...
        dir1, type1, dir2, type2 = _colonization_split(current_section, ang, self.params, self.context)

        first_point = np.array(current_section.last_point)

        stop1, stop2 = self.get_stop_criteria(current_section)

        return section_data(dir1, first_point, stop1, type1), \
